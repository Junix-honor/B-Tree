...
// B+tree
const short NODE_TYPE_ROOT = 0;          // 根节点
const short NODE_TYPE_INTERNAL = 1;      // 普通的中间节点
const short NODE_TYPE_POINT_TO_LEAF = 3; // 指向叶子节点的中间节点
...
class Root
{
  ...
  public:
    ...
    {
        cnt = htobe32(cnt);
        ::memcpy(buffer_ + ROOT_BLOCKCNT_OFFSET, &cnt, ROOT_BLOCKCNT_SIZE);
    }
    ...
    {
        unsigned int cnt;
        ::memcpy(&cnt, buffer_ + ROOT_BLOCKCNT_OFFSET, ROOT_BLOCKCNT_SIZE);
        cnt = be32toh(cnt);
        return cnt;
    }
...
class Block
{
  public:
    ...
    static const int BLOCK_USEDSPACE_OFFSET =
        BLOCK_FREESPACE_OFFSET + BLOCK_FREESPACE_SIZE; //已用空间大小偏移量
    static const int BLOCK_USEDSPACE_SIZE = 4;         // 已用空间大小4B

    static const short BLOCK_DEFAULT_FREESPACE =
        BLOCK_USEDSPACE_OFFSET + BLOCK_USEDSPACE_SIZE + 4; // 空闲空间缺省偏移量
    static const int BLOCK_DEFAULT_CHECKSUM = 0xc70f393e;

    static const int INITIAL_FREE_SPACE_SIZE =
        BLOCK_CHECKSUM_OFFSET - BLOCK_DEFAULT_FREESPACE; //初始空闲空间大小
  ...
  public:
    ...
    // 获取usedspace
    inline int getUsedspace()
    {
        int space;
        ::memcpy(
            &space, buffer_ + BLOCK_USEDSPACE_OFFSET, BLOCK_USEDSPACE_SIZE);
        return be32toh(space);
    }

    // 设定usedspace
    inline void setUsedspace(int space)
    {
        space = htobe32(space);
        ::memcpy(
            buffer_ + BLOCK_USEDSPACE_OFFSET, &space, BLOCK_USEDSPACE_SIZE);
    }
    ...
    virtual bool allocate(const unsigned char *header, struct iovec *iov, int iovcnt);

    // 删除record
    virtual int recDelete(struct iovec *keyField, RelationInfo *relationInfo);
...
class MetaBlock : public Block
{
  public:
    ...
    static const int META_TABLES_OFFSET =
        BLOCK_USEDSPACE_OFFSET + BLOCK_USEDSPACE_SIZE; // 表个数偏移量
...
class DataBlock : public Block
{
  public:
    ...
    static const int DATA_ROWS_OFFSET =
        BLOCK_USEDSPACE_OFFSET + BLOCK_USEDSPACE_SIZE; // 记录个数偏移量
    ...
    static const int INITIAL_FREE_SPACE_SIZE =
        BLOCK_CHECKSUM_OFFSET - DATA_DEFAULT_FREESPACE; //初始空闲空间大小

  public:
    void clear(unsigned int blockid);
    bool allocate(const unsigned char *header, struct iovec *iov, int iovcnt);
    int recDelete(struct iovec *keyField, RelationInfo *relationInfo);
    int rewrite();
    // 获得记录个数
...
class IndexBlock : public Block
{
  public:
    ...
    static const int INDEX_ROWS_OFFSET =
        BLOCK_USEDSPACE_OFFSET + BLOCK_USEDSPACE_SIZE; // 记录个数偏移量
    static const int INDEX_ROWS_SIZE = 4;              // 记录个数大小4B

    static const int NODE_TYPE_OFFSET =
        INDEX_ROWS_OFFSET + INDEX_ROWS_SIZE; // node类型偏移量
    static const int NODE_TYPE_SIZE = 2;     // node类型大小2B

    static const short INDEX_DEFAULT_FREESPACE =
        NODE_TYPE_OFFSET + NODE_TYPE_SIZE + 2; // 空闲空间缺省偏移量
    static const int INDEX_INDEX_START =
        NODE_TYPE_OFFSET + NODE_TYPE_SIZE + 2; // 记录开始位置

    static const int INITIAL_FREE_SPACE_SIZE =
        BLOCK_CHECKSUM_OFFSET - INDEX_DEFAULT_FREESPACE; //初始空闲空间大小
  ...
  public:
    ...
    {
        unsigned int count;
        ::memcpy(&count, buffer_ + INDEX_ROWS_OFFSET, INDEX_ROWS_SIZE);
        return be32toh(count);
    }
    // 设定记录个数
    inline void setRowCount(unsigned int count)
    {
        count = htobe32(count);
        ::memcpy(buffer_ + INDEX_ROWS_OFFSET, &count, INDEX_ROWS_SIZE);
    }

    // 获取节点类型
    inline unsigned short getNodeType()
    {
        unsigned short type;
        ::memcpy(&type, buffer_ + NODE_TYPE_OFFSET, NODE_TYPE_SIZE);
        return be16toh(type);
    }
    // 设定节点类型
    inline void setNodeType(unsigned short type)
    {
        type = htobe16(type);
        ::memcpy(buffer_ + NODE_TYPE_OFFSET, &type, NODE_TYPE_SIZE);
    }
};
} // namespace db
