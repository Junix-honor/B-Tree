...
int Schema::loadData(TableSpace::iterator it)
{
    return it->second.dataFile.open(it->second.dataPath.c_str());
}

int Schema::loadIndex(TableSpace::iterator it)
{
    return it->second.indexFile.open(it->second.indexPath.c_str());
}
...
void Schema::initIov(const char *table, RelationInfo &info, struct iovec *iov)
{
    ...
    iov[1].iov_base = (void *) info.dataPath.c_str();
    iov[1].iov_len = info.dataPath.size() + 1;
    iov[2].iov_base = (void *) info.indexPath.c_str();
    iov[2].iov_len = info.indexPath.size() + 1;
    unsigned short count = info.count;
    info.count = htobe16(info.count); // 底层保存为big endian
    iov[3].iov_base = &info.count;
    iov[3].iov_len = sizeof(unsigned short);
    info.type = htobe16(info.type);
    iov[4].iov_base = &info.type;
    iov[4].iov_len = sizeof(unsigned short);
    info.key = htobe32(info.key);
    iov[5].iov_base = &info.key;
    iov[5].iov_len = sizeof(unsigned int);
    iov[6].iov_base = &info.size; // 初始化为0，不需要转化为big endian
    iov[6].iov_len = sizeof(unsigned long long);
    iov[7].iov_base = &info.rows; // 初始化为0，不需要转化为big endian
    iov[7].iov_len = sizeof(unsigned long long);
    // 初始化field
    size_t index = 8;
    for (unsigned short i = 0; i < count; ++i) {
    ...
    int iovcnt)
{
    ...
    info.dataPath = (const char *) iov[1].iov_base;
    info.indexPath = (const char *) iov[2].iov_base;
    ::memcpy(&info.count, iov[3].iov_base, sizeof(unsigned short));
    info.count = be16toh(info.count);
    ::memcpy(&info.type, iov[4].iov_base, sizeof(unsigned short));
    info.type = be16toh(info.type);
    ::memcpy(&info.key, iov[5].iov_base, sizeof(unsigned int));
    info.key = be32toh(info.key);
    ::memcpy(&info.size, iov[6].iov_base, sizeof(unsigned long long));
    info.size = be64toh(info.size);
    ::memcpy(&info.rows, iov[7].iov_base, sizeof(unsigned long long));
    info.rows = be64toh(info.rows);
    int count = (iovcnt - 8) / 4;
    info.fields.clear();
    for (int i = 0; i < count; ++i) {
        FieldInfo field;
        field.name = (const char *) iov[8 + i * 4].iov_base;
        ::memcpy(
            &field.index,
            iov[8 + i * 4 + 1].iov_base,
            sizeof(unsigned long long));
        field.index = be64toh(field.index);
        ::memcpy(&field.length, iov[8 + i * 4 + 2].iov_base, sizeof(long long));
        field.length = be64toh(field.length);
        field.fieldType=(const char *) iov[8 + i * 4 + 3].iov_base;
